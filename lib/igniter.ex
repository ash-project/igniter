defmodule Igniter do
  ...
  * `:yes` - If `true`, automatically applies the changes without prompting the user.
  ...
  # This line limit is very high, but the file writing experience is not great
  ...
  defp handle_long_diff(igniter, opts) do
    ...
    options = [
      write: "Proceed without viewing changes.",
      display: "Display the diff inline anyway.",
      patch_file: "Write to the `.igniter` file."
    ]

    Igniter.Util.IO.select(
      "Too many changes to automatically display a full diff. The following files will be changed:\n\n" <>
    ...
    |> case do
      ...
      :write ->
        Mix.shell().info(
          "Diff:\n\n#{IO.ANSI.yellow()}Not shown due to line count.#{IO.ANSI.reset()}"
        )
